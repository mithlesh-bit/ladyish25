    ...
    const fetchDesigns = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/coupon/list`
        );
        if (!response.ok) {
                ...
                {designs.map((design) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <span className="whitespace-nowrap  overflow-x-auto block scrollbar-hide text-black text-sm">
                        {design?.codeName}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="whitespace-nowrap scrollbar-hide text-sm text-black">
                        {design?.discountPercentage}%
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="whitespace-nowrap scrollbar-hide text-sm">
                        {design?.minimumCartValue}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="whitespace-nowrap scrollbar-hide text-sm">
                        {design?.validity
                          ? new Intl.DateTimeFormat("en-GB", {
                              ...
                              month: "2-digit",
                              year: "numeric",
                            }).format(new Date(design.validity))
                          : "Invalid Date"}
                      </span>
                    </td>
                    <td className="px-6 py-4">
...
function DeleteProduct({ id }) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/coupon/delete/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <>
      <div className="flex items-center justify-center h-full">
        <button
          type="button"
          className="bg-red-50 border items-center flex border-red-900 p-0.5 rounded-secondary text-red-900"
...
//                 </div>
//               </div>
//               <p className="select-none text-sm whitespace-normal">{review?.comment}</p>
